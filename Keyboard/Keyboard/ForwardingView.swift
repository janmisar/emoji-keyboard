...
class ForwardingView: UIView {
    ...
    var touchToView: [UITouch: KeyButton] = [:]
    ...
    func findNearestView(_ position: CGPoint) -> KeyButton? {
        if !self.bounds.contains(position) {
            return nil
        }
        
        var closest: (KeyButton, CGFloat)? = nil
        
        for anyView in keys {
            let view = anyView
            
            let distance = distanceBetween(view.convert(view.bounds, to: self), point: position)
    ...
    func ownView(_ newTouch: UITouch, viewToOwn: KeyButton?) -> Bool {
        var foundView = false
